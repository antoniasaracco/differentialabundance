...
workflow DIFFERENTIALABUNDANCE {
    ...
    ch_report_input = ch_report_files    // [meta, report_file, logo_file, css_file, citations_file]
        ...
        .filter { tuple -> tuple.size() < 3 || tuple[1] != null }
        .map { tuple ->
            [tuple[0], tuple.tail().collectMany { v -> (v instanceof List) ? v : [v] }.findAll { it != null }]
        }  // [meta, [report_file, logo, css, citations, versions_file, ...]]
        .flatMap { meta, files ->
            def report_files = files[0]
            def report_paths = (report_files instanceof List) ? report_files : [report_files]
            def other_files = files.tail()
            // Split comma-separated report files and create separate entries for each
           report_paths.collect { report_path ->
                def report_file_obj = file(report_path.toString().trim(), checkIfExists: true)
                [meta, report_file_obj, other_files]
            }
        }
